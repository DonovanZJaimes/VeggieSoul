//
//  RecipeListLineView.swift
//  VeggieSoul
//
//  Created by Donovan on 12/07/23.
//

//MARK: INFO:
/*Creacion de una vista que servira como separador de las recetas de los ingredientesde la vista  "RecipeListByIngredient"*/

import Foundation
import UIKit


class RecipeListLineView: UICollectionReusableView {
    
    static let reuseIdentifier = "LineView" /***Agregamos un identificador para la vista***/
    
    
    //MARK: Creacion del marco de la vista
    override init(frame: CGRect) {
        super.init(frame: frame)
        backgroundColor = .lightGray
    }
    
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    //MARK: metodo para cambiar el color de la inea de separacion
    func setColor(_ color: UIColor) {
        backgroundColor = color
    }
    
}
